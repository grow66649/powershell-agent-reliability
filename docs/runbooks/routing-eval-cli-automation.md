...
The prompt is read from the frozen manifest file and sent once through stdin. There is no follow-up steering. Raw CLI stdout JSONL and bounded stderr stay under the host-local evidence root. The temporary secret-bearing profile is deleted after the row.
