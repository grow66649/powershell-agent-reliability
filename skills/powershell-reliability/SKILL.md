...
description: Use this skill only after an execution that is part of the current user-requested task has an eligible Windows Codex Desktop PowerShell or native-command failure: non-zero exit, timeout/cancellation ambiguity, parser/binding/command-resolution failure, declared shell/cwd/executable identity mismatch, or an explicit task post-condition that is false even when exit code is 0. Do not use before that task attempt, for historical, descriptive, or hypothetical failure context with no current task execution failure, for known-good successful work, for ordinary PowerShell explanation/code-writing, for failures from an optional inspection or probe you chose when the requested task itself has not failed, for unrelated MCP/connector/backend failures, for stderr-only warnings when the required task outcome passed, or merely because multiple PowerShell versions are installed.
---
...
3. Freeze the deterministic task criteria before diagnosis or repair. Expected hashes, sizes, names, and other target values must come from the user/task or declared target, never from repaired candidate output. If the task requires exact content, bytes, or hash, preserve an equivalent exact check; existence or size alone is not a substitute.
...
6. Keep probe/helper failures separate from the original task failure unless the probe is itself the failed task boundary. If an optional inspection or probe you chose fails while the requested task itself has not failed, continue the requested task normally with zero Reliability calls.
